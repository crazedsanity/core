...
class TestOfCSFileSystem extends PHPUnit_Framework_TestCase {
	...
	public function test_basics() {
		...
		// check that specifying a valid current working directory (cwd) works
		{
			...
			$validCwd2 = new FileSystem(dirname(__FILE__), '/files');
			...
			$validCwd = new FileSystem(dirname(__FILE__), 'files');
		...
		{
			$invalidCwd = new FileSystem(dirname(__FILE__), '/xDoEsn0tEx15t');
		...
		//test that it fixes invalid modes.
		{
			...
			foreach($validModes as $x) {
				$testMe = new FileSystem(dirname(__FILE__), null, $x);
